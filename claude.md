...
- Go 1.25.x，模块名为 `vpk-manager`
- Wails v2.10.2
- 前端：Vite 3、原生 JavaScript、按功能拆分的 CSS
- 发布：推送 `v*` tag 后由 GitHub Actions 在 Windows 环境构建
- 主要依赖：
  - `l4d2-manager-next/pkg/valve/vpk`、`l4d2-manager-next/pkg/vpkmission`：VPK 与 mission 文件解析
...
发布流程在推送 `v*` tag 时触发，运行环境为 `windows-latest`，使用 Go 1.25.x、Node.js 20 和 Wails v2.10.2。核心构建命令为：
